...
/project
  ...
  /services
     /Beamablefootboll
        Beamablefootboll.csproj
        Dockerfile
        Program.cs
        Beamablefootboll.cs
     /BeamablefootbollLibrary
        BeamablefootbollLibrary.csproj
        BeamablefootbollLibrary.cs
```

## Running Locally
The Beamablefootboll project can be run locally by starting it from an IDE, or by executing the `dotnet run` command.
...
```shell
beam services deploy --ids Beamablefootboll
```

### MSBuild Properties
The `Beamablefootboll.csproj` file has a few configuration options that can be set.
```xml
<PropertyGroup>
...
#### BeamableTool
By default, both Beamablefootboll and BeamablefootbollLibrary have the BeamCLI installed as a project tool. 
...
remote swagger. 
```shell
beam project open-swagger Beamablefootboll
```
...
at each linked Unity project's `Assets/Beamable/Microservice/Microservices/BeamablefootbollClient.cs`. 

Additionally, the `share-code` msbuild target in the BeamablefootbollLibrary project will copy their
